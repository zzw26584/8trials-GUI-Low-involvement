...
const App: React.FC = () => {
  // 修改处：使用 .reverse() 实现倒序呈现
  const trials = useMemo(() => [...TRIALS].reverse(), []);
  ...
  const startExperiment = () => {
    if (participant.id && participant.runNumber && participant.gender && participant.age) {
      setStage(Stage.GENERAL_INSTRUCTION);
    }
  };

  const proceedToFirstTrial = () => {
  ...
  if (stage === Stage.GENERAL_INSTRUCTION) {
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 text-slate-900">
        <div className="max-w-xl w-full bg-white rounded-[3rem] shadow-2xl p-12 border border-white space-y-10 animate-in">
          <div className="space-y-4">
            <div className="w-16 h-1.5 bg-blue-600 rounded-full mb-8"></div>
            <h2 className="text-4xl font-black leading-tight tracking-tight">实验说明</h2>
            <div className="h-px w-full bg-slate-100"></div>
          </div>
          
          <div className="space-y-6 text-lg font-bold leading-relaxed text-slate-600">
            <p>
              随后的一系列任务是你<span className="text-blue-600 underline underline-offset-8 decoration-4">生活中经常遇到的一些小事</span>。
            </p>
            <div className="bg-blue-50 border-2 border-blue-100 p-8 rounded-[2rem] shadow-sm">
              <p className="text-slate-900 font-black italic text-xl leading-relaxed">
                “请尽可能代入真实生活，提示根据条件，做出符合你日常生活的选择。”
              </p>
            </div>
          </div>

          <button 
            onClick={proceedToFirstTrial} 
            className="w-full bg-slate-900 hover:bg-slate-800 text-white py-6 rounded-2xl font-black text-2xl shadow-2xl shadow-slate-200 active:scale-95 transition-all flex items-center justify-center gap-3"
          >
            我明白了，开始实验
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="max-w-xl w-full bg-white rounded-[2.5rem] shadow-xl p-10 space-y-8 animate-in">
          <div className="text-center space-y-3">
             <h2 className="text-2xl font-black text-slate-900">决策任务 ({trialIndex + 1}/{trials.length})</h2>
      ...
      <div className="min-h-screen bg-slate-50 p-6 overflow-y-auto custom-scrollbar">
        <div className="max-w-xl mx-auto space-y-8 pb-20 pt-8">
          ...
          <div className="bg-white p-8 md:p-12 rounded-[3rem] shadow-2xl border border-white space-y-10">
            ...
            <LikertScale label="2. 您认为自己有多擅于做这项任务？" value={survey.skillLevel} onChange={val => setSurvey({...survey, skillLevel: val})} />
            <LikertScale label="3. 您对执行任务所使用模态的满意度如何？" value={survey.satisfaction} onChange={val => setSurvey({...survey, satisfaction: val})} />
            <LikertScale label="4. 您觉得刚才的对比/决策过程是否高效？" value={survey.efficiency} onChange={val => setSurvey({...survey, efficiency: val})} />
            <LikertScale label="5. 您对本轮用这种方式提供的信息信赖程度如何？" value={survey.trust} onChange={val => setSurvey({...survey, trust: val})} />
            ...
            >
              {trialIndex < trials.length - 1 ? "保存并进入下一试次" : "完成实验并提交数据"}
            </button>
          </div>
        </div>
      </div>
